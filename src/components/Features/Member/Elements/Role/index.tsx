...
export default function Role() {
  return (
    <>
      ...
      <ScrollAnimation variant="staggerContainer" delay={0.1}>
        <ul className="mx-auto mt-10 grid max-w-2xl grid-cols-1 gap-6 sm:grid-cols-2 lg:mx-0 lg:max-w-none lg:grid-cols-3 lg:gap-8">
          {PEOPLE.map((item, index) => (
            <ScrollAnimation key={item.name} variant="staggerItem" delay={index * 0.05}>
              <li className="rounded-2xl px-8 py-10 border border-gray-300">
                <img
                  alt="人物"
                  src={item.path}
